...
def download_nonyt_videos(indexfile, saveto='raw_videos'):
    ...
    for entry in content:
        ...
        for inst in instances:
            ...
            mp4_path = os.path.join(saveto, '{}.mp4'.format(video_id))
            swf_path = os.path.join(saveto, '{}.swf'.format(video_id))
            
            if os.path.exists(mp4_path) or os.path.exists(swf_path):
                logging.info('⏭️  Video {} already exists, skipping download'.format(video_id))
                continue
